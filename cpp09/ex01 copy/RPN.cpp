...
// もし空白文字であれば、token 文字列が空でないかを確認します。もし空でない場合、完全なトークンが蓄積されていることを意味し、token を tokens ベクターに追加し、次のトークンのために token 文字列をクリアします。
...
// トークンが "-" の場合、スタックから2つのオペランドを取り出し、それらを引いて結果をスタックにプッシュします。
// トークンが "*" の場合、スタックから2つのオペランドを取り出し、それらを掛けて結果をスタックにプッシュします。
// トークンが "/" の場合、スタックから2つのオペランドを取り出し、それらを割って結果をスタックにプッシュします。ただし、0で割る場合は例外をスローします。
...
// スタックのトップに残っている値が評価結果となり、それを返します。
// この関数は、逆ポーランド記法の数式を解析し、スタックを使って演算を行い、
